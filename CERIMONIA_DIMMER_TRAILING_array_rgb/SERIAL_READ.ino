...
void processaString() {
  ...
  if (DataIn.indexOf("R_DIMER") == 0)
  {
    ...
    //no modo normal é daqui pra cima
    digitalWrite(AC_pin[canal - 1], intensidade);
    Serial.print("DIM,");
    Serial.print(canal);
    Serial.print(",");
    Serial.println(intensidade);
  ...
  if (DataIn.indexOf("CORT") == 0)
  {
    int index1 = DataIn.indexOf('_');
    int index2 = DataIn.indexOf('_', index1 + 1);
    if (DataIn.substring(index1 + 1, index2) == "ALL") {
      byte acao = DataIn.substring(index2 + 1, DataIn.length() - 1).toInt(); // 0 = fecha; 1 = abre; 2 = para
      if (acao < 0 && acao > 2) return;
      switch (acao) {
        case 0:
          for (int a = 0; a < 5; a++) {
    ...
    int canal = DataIn.substring(index1 + 1, index2).toInt(); // de 1 a 5
    byte acao = DataIn.substring(index2 + 1, DataIn.length() - 1).toInt(); // 0 = fecha; 1 = abre; 2 = para
    if (canal < 1 && canal > 5) return;
    if (acao < 0 && acao > 2) return;
    canal = canal - 1;
    previousMillis[canal] = millis();
    switch (acao) {
      case 0:
        digitalWrite(cortinas[canal][acao + 1], HIGH);
        delay(30);
        digitalWrite(cortinas[canal][acao], LOW);
        break;
      case 1:
        digitalWrite(cortinas[canal][acao - 1], HIGH);
        delay(30);
        ...
        break;
      case 2:
        digitalWrite(cortinas[canal][0], HIGH);
        digitalWrite(cortinas[canal][1], HIGH);
        break;
    }
  ...
  if (DataIn.indexOf("DIMER") == 0)
  {
    ...
    canal = constrain(canal, 1, 10);
    intensidade = constrain(intensidade, 0, 100);
    int value = map(intensidade, 0, 100, 0, 75);
    powers[canal - 1] = value;
