...
#                 self.fade_data()
    ...
    def fade_data(self):
        ...
        for o, n in zip(self.data, self.new_data):
            ...
            if o < n:
                ...
                if self.data[counter] > self.minimum + self.increase:
                    self.data[counter] = self.data[counter] - self.increase
                else:
                    self.data[counter] = self.data[counter] - 1
            counter += 1
    ...
    def loop1(self):
        if self.data[3] == self.maximo:
           self.new_data[1:self.channel_num+1] = np.full((512), self.minimum, dtype=int)
           print(self.data, len(self.data))
           
            
        elif self.data[3] == 0:
            self.new_data[1:self.channel_num+1] = np.full((512), self.maximo, dtype=int)
            print(self.data, len(self.data))
